...
class OpenDTAM
{
	...
	{   
        ...
        if(pyrLevels==0)
		{//auto size to end at >=15px tall (use height because shortest dim usually)
		...
		thread = boost::thread(boost::bind(&OpenDTAM::Tutrk, this));
...
bool utrk(Ptr<Frame> _frame){
    ...
    for (int level=utrkLevel3DStart; level<utrkLevel3DEnd; level++){
        ...
        int iters=3;
        for(int i=0;i<iters;i++){
            bool success = align_level_largedef_gray_forward(   
				(*base.pyramid)[level],//Total Mem cost ~185 load/stores of image
...
cv::cuda::HostMem imageContainerUcv;
bool ucv(Ptr<Frame> _base,Ptr<Frame> _alt){
	...
	Ptr<DepthmapDenoiseWeightedHuber> optimizerp = createDepthmapDenoiseWeightedHuber();//(new DepthmapDenoiseWeightedHuber(cv));
