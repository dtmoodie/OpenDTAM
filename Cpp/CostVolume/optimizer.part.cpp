...
//     _g=1;
    ...
    for (st point=0; point<end; point++){
        gu[here]= 0.5f*(g[up]+   g[here]);
        gd[here]=-0.5f*(g[down]+ g[here]);
        gl[here]= 0.5f*(g[left ]+ g[here]);
        gr[here]=-0.5f*(g[right]+g[here]);
    }
...
static inline float afunc(float* data,float theta,float d,float ds,int a,float lambda){
    return 1.0f/(2.0f*theta)*ds*ds*(d-a)*(d-a) + data[a]*lambda;//Literal implementation of Eq.14, note the datastep^2 factor to scale correctly
...
inline float Cost::aBasic(float* data,float l,float ds,float d,float& value){
    ...
    B=mv*(1.0f -1.0e-8);//avoid divide by zero, since B is already <= others, make < others
...
static boost::thread threadQD;
static boost::thread threadA;

static void launch_optimzer_threads(const Cost* cost){
    std::cout << "Lauching optimizer threads..." << std::endl;

	threadQD = boost::thread(boost::bind(&Cost_optimizeQD, (void*)cost));
	threadA = boost::thread(boost::bind(&Cost_optimizeA, (void*)cost));
